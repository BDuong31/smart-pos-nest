...
export const ErrUsernameOrPasswordInvalid = new Error(
...
export const ErrUsernameInvalidFormat = new Error(
  'Username has invalid format',
); // Lỗi định dạng tên người dùng không hợp lệ
...
export const ErrBirthdayInFuture = new Error(
  'Birthday cannot be in the future',
); // Lỗi ngày sinh không thể ở tương lai
export const ErrUserUnderAge = new Error(
  'User is under the minimum age requirement',
); // Lỗi người dùng chưa đủ tuổi
...
export const userSchema = z.object({
  ...
  birthday: z.coerce
    .date({ message: ErrBirthdayInvalid.message }) // Ngày sinh hợp lệ và bắt buộc
    .refine((date) => date <= new Date(), {
      message: ErrBirthdayInFuture.message,
...
}; // Kiểu dữ liệu User được suy luận từ schema
