...
export interface IStockCheckService {
  create(
    requester: Requester,
    dto: StockCheckCreateDTO,
    ip: string,
    userAgent: string,
  ): Promise<StockCheck>; // Tạo phiếu kiểm kho mới
  update(
    requester: Requester,
    stockCheckId: string,
    dto: StockCheckUpdateDTO,
    ip: string,
    userAgent: string,
  ): Promise<StockCheck>; // Cập nhật thông tin phiếu kiểm kho theo ID
  delete(
    requester: Requester,
    stockCheckId: string,
    ip: string,
    userAgent: string,
  ): Promise<void>; // Xóa phiếu kiểm kho theo ID

  get(stockCheckId: string): Promise<StockCheck | null>; // Lấy thông tin phiếu kiểm kho theo ID
  list(
    cond: StockCheckCondDTO,
    pagingDTO: PagingDTO,
  ): Promise<Paginated<StockCheck>>; // Lấy danh sách phiếu kiểm kho theo điều kiện
