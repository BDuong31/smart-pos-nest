...
import type {
  StockCheckDetailCreateDTO,
  StockCheckDetailUpdateDTO,
  StockCheckDetailCondDTO,
} from '../dtos/stockCheckDetail.dto';
...
export interface IStockCheckDetailService {
  create(
    requester: Requester,
    dto: StockCheckDetailCreateDTO,
    ip: string,
    userAgent: string,
  ): Promise<StockCheckDetail>; // Tạo chi tiết phiếu kiểm kho mới
  update(
    requester: Requester,
    stockCheckDetailId: string,
    dto: StockCheckDetailUpdateDTO,
    ip: string,
    userAgent: string,
  ): Promise<StockCheckDetail>; // Cập nhật thông tin chi tiết phiếu kiểm kho theo ID
  delete(
    requester: Requester,
    stockCheckDetailId: string,
    ip: string,
    userAgent: string,
  ): Promise<void>; // Xóa chi tiết phiếu kiểm kho theo ID

  get(stockCheckDetailId: string): Promise<StockCheckDetail | null>; // Lấy thông tin chi tiết phiếu kiểm kho theo ID
  list(
    cond: StockCheckDetailCondDTO,
    pagingDTO: PagingDTO,
  ): Promise<Paginated<StockCheckDetail>>; // Lấy danh sách chi tiết phiếu kiểm kho theo điều kiện
  listByIds(ids: string[]): Promise<StockCheckDetail[]>; // Lấy danh sách chi tiết phiếu kiểm kho theo nhiều ID
}
...
export interface IStockCheckDetailRepository {
  ...
  update(
    stockCheckDetailId: string,
    dto: StockCheckDetailUpdateDTO,
  ): Promise<void>; // Cập nhật thông tin chi tiết phiếu kiểm kho theo ID
