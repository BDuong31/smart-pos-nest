            ...
            async (event: any) => {
                ...
                if (evt.payload.status === 'pending') {
                    ...
                    await this.notificationService.createNotification(
                        ...
                        {
                            title: 'Đơn hàng mới',
                            content: `Đơn hàng #${evt.payload.code} đã được tạo. Vui lòng kiểm tra và xử lý đơn hàng.`,
                            role: UserRole.STAFF,
                    ...
                    await this.notificationService.createNotification(
                        { sub: evt.payload.userId || 'system', role: UserRole.CUSTOMER },
                        {
                            ...
                            content: `Đơn hàng #${evt.payload.code} của bạn đã được tạo thành công. Vui lòng chờ nhân viên xác nhận.`,
                ...
                if (evt.payload.status === 'confirmed') {
                    ...
                    await this.notificationService.createNotification(
                        { sub: evt.payload.userId || 'system', role: UserRole.CUSTOMER },
                        {
                            ...
                            content: `Đơn hàng #${evt.payload.code} của bạn đã được nhân viên tạo thành công. Món ăn đang được chuẩn bị. Vui lòng chờ trong giây lát.`, 
                            userId: evt.payload.userId, 
                            type: NotificationType.ORDER,   
                            refType: evt.payload.changeType,    
                            refId: evt.payload.orderId,
                    ...
                    await this.notificationService.createNotification(  
                        { sub: 'system', role: UserRole.ADMIN },    
                        {
                            ...
                            content: `Đơn hàng #${evt.payload.code} đã được nhân viên tạo thành công. Vui lòng kiểm tra và xử lý đơn hàng.`,   
                            role: UserRole.STAFF,   
                            type: NotificationType.ORDER,    
                            refType: evt.payload.changeType,    
                            refId: evt.payload.orderId, 
                    ...
                    await this.notificationService.createNotification(
                        { sub: 'system', role: UserRole.KITCHEN },
                        {
                            ...
                            content: `Đơn hàng #${evt.payload.code} đã nhân viên tạo thành công và cần được chuẩn bị. Vui lòng kiểm tra và bắt đầu chuẩn bị món ăn.`, 
            ...
            async (event: any) => {
                ...
                if (evt.payload.status === 'confirmed') {
                    ...
                    await this.notificationService.createNotification(
                        ...
                        {
                            title: 'Đơn hàng của bạn đã được xác nhận',
                            content: `Đơn hàng #${evt.payload.code} của bạn đã được nhân viên xác nhận. Món ăn đang được chuẩn bị. Vui lòng chờ trong giây lát.`,
                            userId: evt.payload.userId,
                            type: NotificationType.ORDER,
                            refType: evt.payload.changeType,
                            refId: evt.payload.orderId,
                    ...
                    await this.notificationService.createNotification(
                        { sub: 'system', role: UserRole.KITCHEN },
                        {
                            ...
                            content: `Đơn hàng #${evt.payload.code} đã được nhân viên xác nhận và cần được chuẩn bị. Vui lòng kiểm tra và bắt đầu chuẩn bị món ăn.`,
                            role: UserRole.KITCHEN,
                            type: NotificationType.ORDER,
                            refType: evt.payload.changeType,
                            refId: evt.payload.orderId,
                ...
                if (evt.payload.status === 'processing') {
                    ...
                    await this.notificationService.createNotification(
                        { sub: evt.payload.userId || 'system', role: UserRole.CUSTOMER },
                        {
                            ...
                            content: `Đơn hàng #${evt.payload.code} của bạn đang được chuẩn bị. Vui lòng chờ trong giây lát.`,
                            userId: evt.payload.userId,
                            type: NotificationType.ORDER,
                            refType: evt.payload.changeType,
                            refId: evt.payload.orderId,
                    ...
                    await this.notificationService.createNotification(
                        { sub: 'system', role: UserRole.STAFF },
                        {
                            ...
                            content: `Đơn hàng #${evt.payload.code} đang được chuẩn bị. Vui lòng kiểm tra tiến độ và sẵn sàng phục vụ khách hàng khi món ăn đã hoàn thành.`,
                            role: UserRole.STAFF,
                            type: NotificationType.ORDER,
                            refType: evt.payload.changeType,
                            refId: evt.payload.orderId,
                ...
                if (evt.payload.status === 'served') {
                    ...
                    await this.notificationService.createNotification(
                        { sub: evt.payload.userId || 'system', role: UserRole.CUSTOMER },
                        {
                            ...
                            content: `Đơn hàng #${evt.payload.code} của bạn đã được chuẩn bị xong và đang được phục vụ. Vui lòng chuẩn bị thanh toán khi nhân viên đến bàn bạn.`,
                            userId: evt.payload.userId,
                            type: NotificationType.ORDER,
                            refType: evt.payload.changeType,
                            refId: evt.payload.orderId,
                    ...
                    await this.notificationService.createNotification(
                        { sub: 'system', role: UserRole.STAFF },
                        {
                            ...
                            content: `Đơn hàng #${evt.payload.code} đã được chuẩn bị xong và đang được phục vụ. Vui lòng chuẩn bị thanh toán khi nhân viên đến bàn khách hàng.`,
                            role: UserRole.STAFF,
                            type: NotificationType.ORDER,
                            refType: evt.payload.changeType,
                            refId: evt.payload.orderId,
                ...
                if (evt.payload.status === 'cancelled') {
                    ...
                    await this.notificationService.createNotification(
                        { sub: evt.payload.userId || 'system', role: UserRole.CUSTOMER },
                        {
                            ...
                            content: `Đơn hàng #${evt.payload.code} của bạn đã bị hủy. Nếu bạn có thắc mắc, vui lòng liên hệ với nhân viên để được hỗ trợ.`,
                            userId: evt.payload.userId,
                            type: NotificationType.ORDER,
                            refType: evt.payload.changeType,
                            refId: evt.payload.orderId,
                    ...
                    await this.notificationService.createNotification(
                        { sub: 'system', role: UserRole.STAFF },
                        {
                            ...
                            content: `Đơn hàng #${evt.payload.code} đã bị hủy. Vui lòng kiểm tra lý do hủy và liên hệ với khách hàng nếu cần thiết.`,
                            role: UserRole.STAFF,
                            type: NotificationType.ORDER,
                            refType: evt.payload.changeType,
                            refId: evt.payload.orderId,
                    ...
                    await this.notificationService.createNotification(
                        ...
                        {
                            title: 'Đơn hàng đã bị hủy',
                            content: `Đơn hàng #${evt.payload.code} đã bị hủy. Vui lòng kiểm tra lý do hủy và ngừng chuẩn bị món ăn nếu bạn đang chuẩn bị.`,
                            role: UserRole.KITCHEN,
                            type: NotificationType.ORDER,
                            refType: evt.payload.changeType,
                            refId: evt.payload.orderId,
